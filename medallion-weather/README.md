...
You can start writing assets in `medallion_weather/assets/`. The assets are automatically loaded into the Dagster repository as you define them.
...
Tests are in the `medallion_weather_tests` directory and you can run tests using `pytest`:

```bash
pytest medallion_weather_tests
```
